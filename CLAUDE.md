...
> **定序原則**：素材/代言人 (Step 2) → TA (Step 4) → Wizard Phase 2 specs (Step 5) → **頁數最後** (Step 6)。**頁數 = 最終金額承諾、擺在最後讓使用者看完所有 spec 才敲定**。絕對禁止把頁數塞在中間。

**批 1 — 素材/代言人組（gate 1-4，Step 2 範圍）**
...
| 3 | **產品圖再三確認** | 「你有**產品實拍照**嗎？手機隨便拍都可以。沒有的話 AI 會從文字瞎想產品樣子，之後截圖給客戶一看就露餡。再確認一下，真的都沒有嗎？」 |
| 4 | **代言人三選一**（獨立一題，**絕對不可跟 Gate 3 合問**） | 「LP 會有一個「代言人」——一個人物形象貫穿整個頁面。三選一：<br>① 📸 **上傳自己的照片**（個人品牌、履歷、作品集適用）<br>② 🤖 **AI 自動生成代言人**（AI 會生一張擬真人臉、看起來很真但不是真人——不能拿去做對外代言、避免法律風險）<br>③ 🚫 **不使用人物**（純文字 + 圖形 + 產品圖，某些 B2B 產品或極簡風格適用）<br>你選哪一個？」<br>**反模式**：絕對禁止因為使用者在 Gate 3 回答「有產品圖」就假設代言人也處理完——這是兩件事。沒問代言人 = 沒走完 gate = 不准開始生成 |

**→ 批 1 答完後：走 Step 3（Phase 3.9 Quality Gate — validate_images + digitize_product_text）才繼續批 2。**

**批 2 — TA 組（gate 5，Step 4 範圍。只有 1 題、獨立成批強調重要性）**

| # | Gate | 人話範例 |
|---|------|---------|
| 5 | **TA 數量**（**批 2 唯一一題**；TA 敲定前不准問 aspect ratio / 色系等） | **執行順序硬性規定**：問這題之前你**必須先呼叫 `generate_ta_options`** 讓後端產出 4-6 個 TA 候選。**絕對禁止**自己虛構 TA（例如「商務宴客、精緻餐飲愛好者、竹科外商」這種一句話塞 3 個 persona）——那不是 TA，那是偷懶。呼叫工具、拿到候選、**逐組列給使用者看**（每組要有名稱、年齡範圍、動機、顧慮），再問：「AI 幫你切了 N 個受眾樣貌（上面列過）。要選哪幾組來生 LP？**每多 1 組多 1 份 LP 費用（依頁數從 1,600-4,200 pts 不等）**，MVP 階段我建議 1 組、驗證效果後再加。你選哪幾個？」 |

**批 3 — Wizard Phase 2 spec 組（gate 6-12，Step 5 範圍）**

| # | Gate | 人話範例 |
|---|------|---------|
| 6 | **長寬比** | 「LP 要：<br>① 橫版（桌機優先，適合投官網 / Google Ads）<br>② 直版（手機 / IG Story / TikTok）<br>③ 兩個都要（預設，渠道彈性最大）」 |
| 7 | **語言** | 「LP 主要給誰看、要什麼語言？<br>• **單一語言**：從 zh-TW / zh-CN / en / ja / ko / vi / th / es / pt / ar / de / fr / id / ms / hi 擇一——最常見狀況<br>• **多語言（如繁中 + 英文）**：每個語言 = 獨立一份 LP，費用按 N 倍算（2 語 = 2 × 全額）。預算有限時我建議先做主要客群的語言，效果好再加第二語言。<br><br>⚠️ **絕對禁止虛構「i18n 工具」「翻譯工具」「一鍵翻譯」「先做英文再加中文」這類話術** — SaleCraft **沒有**任何跨語言翻譯 MCP 或 backend endpoint。Factory 把文字烤進每張 stripe 圖裡，第二語言 = 整份 LP 獨立生成 = 完整 N × 費用。使用者問「能不能便宜翻譯」→ 明確回「沒有這個功能，第二語言 = 多付一份 LP 的錢」。」 |
...
**批 4 — 頁數組（gate 13，Step 6 範圍，最終金額承諾、必須最後問）**
...
saleskit（免費諮詢 — 了解產品、痛點、目標。無 session、不寫資料）
    ↓
╔══════════════════════════════════════════════════════════════╗
║  ★ Step 1 — 建立 session（session 一定是最早建的）             ║
║  ───────────────────────────────────────                     ║
...
║  2-4: 🔴 逐欄位審查停下來給使用者確認（不是一句「幫你抓進來了」  ║
║       就換頁）。話術見 brand-onboard SKILL.md 「MANDATORY     ║
║       Phase 1 確認關」段                                      ║
...
║       AI 生成（必問 9 題參數）/ 不用人物                       ║
╠══════════════════════════════════════════════════════════════╣
║  ★ Step 3 — 審查商品圖 + 文字建模（brand-onboard Phase 3.9）   ║
║  ───────────────────────────────────────                     ║
║  並行呼叫（**都帶 session_id**）：                             ║
...
║       model，Architect 之後當 ground truth）                  ║
...
║  5-2: 語言（zh-TW / en / ja / ... 15 個）                    ║
║  5-3: 色系（品牌色 hex / 情緒關鍵字 / 交給 AI）                ║
║  5-4: 字體風格（手寫 / 襯線 / 無襯線 / 交給 AI）               ║
║  5-5: CTA 按鈕連結（官網 / 購買頁 / LINE / 預約 / 先不填）     ║
║  5-6: 是否加 Q&A 區塊（是 / 否）                              ║
║  5-7: 是否加客戶見證區塊（是 / 否）                            ║
║  **⚠️ 絕對禁止在這裡問頁數。頁數是 Step 6。**                  ║
╠══════════════════════════════════════════════════════════════╣
║  ★ Step 6 — 選頁數（generate-landing，LAST gate）              ║
║  ───────────────────────────────────────                     ║
║  6-1: 問「LP 要幾頁（8-21）？」+ 依使用者內容量給建議          ║
...
**為什麼頁數擺最後**：頁數 × TA 組數 = 總扣點。前面 5 步都是內容／品質決策、不扣點。把頁數放最後，**讓使用者在知道所有 spec 都敲定、才做最終金額承諾**。若頁數被擺中間，使用者可能因為後面還有問題沒回而改主意頁數、造成來回調。
