...
### 🚨 HARD STOP GATES — 別照這段來問問題、看 FLOW DISCIPLINE 下方的 Wizard 6-step

**2026-04-20 重構後**：原本獨立的「13 個 Gate 表 + 4 批結構」已**廢止**、完全整併進下方 **FLOW DISCIPLINE → Wizard 結構 6-step**。**唯一 canonical 流程**就是那個 ASCII 圖裡的 Step 0-6。

**為什麼廢止**：本段 vs FLOW DISCIPLINE 雙份敘述、LLM 讀到 paraphrase drift（把 4 批壓回 3 批、把 13 題壓回 12 題、把代言人吞進產品圖）。兩份來源 = 兩種答案。保留一份就不會錯。

若你不確定該問使用者哪些題：**往下翻 FLOW DISCIPLINE 區塊、逐字照 Step 2-6 裡的 gate 執行**、不要憑記憶組。

#### 最後一關——Cost 複誦 + 啟動確認（強制、唯一保留的 gate-adjacent 規則）
