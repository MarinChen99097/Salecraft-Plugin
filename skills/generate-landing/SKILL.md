...
### 🔴 Step 5 SCOPE WHITELIST — AUTHORITATIVE

Wizard Phase 2 唯一合法欄位（對齊 backend `TAGroupInput` + `wizard_shared_data`、違者 = 幻想欄位）：

**Per-TA**（寫 `wizard_ta_groups[i]`）：`language` / `visual_style` / `primary_color` / `font_family`（中文字體）/ `font_family_english`（英文字體）/ `headline_weight`（400-800、default 700）/ `body_weight`（400-600、default 400）/ `text_color` / `ta_description_images` / `full_description_images` / `tags` / `notes`

**Shared**（寫 `wizard_shared_data`）：`aspect_ratio` / `requested_stripe_count` / `cta_url` / `cta_text`

**以上就是全部**。Step 5 的問題、宣告、Cost 複誦只能出現這些欄位、不准替使用者發明新欄位、不准替使用者重新定義既有欄位語義（例：把 language 包裝成「繁中 / 繁中+英文」二選一 = 違規）。

### 🔴 Step 5 禁問題庫（常見幻想題、backend 沒儲存位置）

以下題目在 Wizard Phase 2 沒儲存位置、提 = 違規：

- LP 版型風格（極簡 / 沉浸 / 資訊密集 / 影院式 / 敘事長頁 等命名流派）—— 不存在、風格由 `visual_style` + Architect 決定
- LP 區塊加選（FAQ / 評價 / 會員 / 節慶活動 / 母品牌導流 / Google Map / 會員儲值）—— phantom fields、backend 不讀、要加區塊走 post-gen edit-landing
- Hero 首屏主圖選哪張 —— Factory 自動挑、Wizard 不介入（要換圖走 post-gen）
- 主標語選哪句 —— Strategist/Architect 生成、不是 Wizard 欄位
- 語氣 Tone of Voice（優雅 / 熱情 / 權威）—— 不存在、tone 隱含在 `visual_style` + TA 描述
- 促銷訊息 / 檔期活動 / 開幕優惠 —— 不存在、要加走 post-gen edit-landing
- 次要 CTA / 第二按鈕 —— 不存在、只有單一 `cta_url` + `cta_text`
- 表單欄位（姓名 / 電話 / 人數 / 時段）—— 不存在、轉換走 `cta_url` 外連
- 追蹤碼（Meta Pixel / GA4 / Google Ads / LINE Tag）—— 是 post-gen `AnalyticsConfig`、不在 Wizard
- 自訂網域 —— post-deploy、不在 Wizard
- `include_qa_section` / `include_testimonials` —— phantom、backend 0 hits、已明令禁用

### 🔴 CTA 禁問紀律

`cta_url` / `cta_text` 走 silent default（URL=brand 官網、text=產業預設：餐飲→「立即訂位」、保健→「立即購買」、服務→「立即諮詢」、general→「了解更多」）。**禁止**向使用者提問、禁止擺選單讓使用者挑、禁止以「你之前說連到 X、我幫你找到的是 Y」這類框架回頭詢問。使用者主動給 URL 才收、post-gen 走 `update_cta_link` 微調。

### 🔴 Step 5 對立失敗模式（兩種都是違規）

- **收縮型**：用「最後三題」「繁中 vs 繁中+英文」「CTA 三選一」等二選一框架把 Wizard 欄位壓縮、跳過 per-TA per-欄位宣告 → 違 NO SILENT DEFAULTS + 違 Bilingual 禁令
- **膨脹型**：無中生有版型 / Hero / 標語 / 語氣 / 促銷 / 追蹤碼 / 網域 / 次要 CTA / 表單 等 10+ 幻想題 → 違 SCOPE WHITELIST

Step 5 正確 footprint：每 TA `language` 必問（8 options）+ 其他 per-TA 欄位走 infer-or-silent-default + Step 5c 一次宣告 **whitelist 內全部欄位**給反對機會 + Step 6 頁數。沒有「第三條路」。
...
| `language` | `wizard_ta_groups[i].language` | **每 TA 各自設**，寫 canonical name（例 A=`"Traditional Chinese (Taiwan)"` / B=`"English"`）、**不寫 ISO code**（UI dropdown 只認 canonical）|
...
| `headline_weight` / `body_weight` / `text_color` | 預設走 silent default | 標題 700、內文 400、文字色 `"auto"`（AI 依背景判斷）。使用者主動要求（例：「字再粗一點」）才改、Step 5c 仍需宣告現值讓使用者反對 |
...
**單 TA 範本**（whitelist 全列）：

```
依你前面講的，幫你把設定預填了：

**共用設定**
- 長寬比：**9:16 直版**（你提過 IG 限時動態）
- CTA 按鈕：文字「立即諮詢」+ 連到你的官網（silent default、之後可改）

**TA1 [TA 名稱]**
- 語言：繁體中文（台灣）（你親答）
- 視覺風格：editorial serif（你做保健食品、襯線適合）
- 品牌色：墨綠 #2fa067（官網抓到的主色）
- 中文字體 / 英文字體：auto + auto（交給 AI 配）
- 標題字重 / 內文字重：700 / 400（預設）
- 文字顏色：auto（AI 依背景判斷）
- 參考圖片：你沒提供 → 空
- 標籤 / 備註：空

有要改哪項直接講哪個 TA 的哪個欄位、或回「都 OK」進最後一題（頁數）。
```

**多 TA（≥2）範本**：**每組 TA 獨立列完整 per-TA 欄位**、共用設定列一次。語言必寫 canonical name（例：`"English"`、不是 `"en"`）、不得兩組同值後標「（我幫你配）」掩蓋沒逐組問。

```
**共用設定**（所有 TA 同）
- 長寬比：[value]（來源）
- CTA：[text] → [url]（silent default）

**TA1 [名稱]**
- 語言：[canonical name]（使用者親答 / infer 來源）
- 視覺風格 / 品牌色 / 字體（中、英、標題字重、內文字重、文字顏色）/ 圖片 / tags / notes
  （每項列值、infer 的標來源）

**TA2 [名稱]**
- 語言：[canonical name]（使用者親答 / infer 來源）
- 視覺風格 / 品牌色 / 字體 / 圖片 / tags / notes

有要改哪組哪項直接講、或回「都 OK」進頁數。
```

**禁止在 Step 5c 出現**：Q&A 區塊 / 客戶見證 / FAQ / 節慶檔期 / 會員儲值 / Hero 主圖 / 主標語 / 語氣 / 促銷 / 次要 CTA / 表單欄位 / 追蹤碼 / 自訂網域 等 whitelist 外欄位——這些是 Step 5 禁問題庫、宣告了也是幻想。
...
每次 `update_session` 寫任何 Step 5 spec 欄位（aspect_ratio / language / primary_color / visual_style / font_family / font_family_english / headline_weight / body_weight / text_color / cta_url / cta_text）後、**立刻 `get_session` 讀回 + 針對該欄位 assert**、禁止繼續往下走直到驗證通過。

**Per-TA 欄位驗證**（language / primary_color / visual_style 等）：

```python
update_session(wizard_ta_groups=[{"id": "ta_2", "language": "English", ...}])
s = get_session(session_id)
ta_2 = next((t for t in s["wizard_ta_groups"] if t.get("id") == "ta_2"), None)
assert ta_2 and ta_2.get("language") == "English", (
...
1. **第一優先、禁止略過：預教育使用者生成完能做什麼**（Beat 1 的完整 template 見下方、列出所有 free post-gen 編輯能力：文字 / 換圖 / logo swap / CTA 顏色與文字 / Footer / Header / 柔邊 / overlay / SEO / **UTM 追蹤分享連結** / 裁切 / 歷史版本）。這是 wait 期間的**主菜**、不是彩蛋
...
`generate_session` 成功後的**下一則訊息**必須 fire Beat 1 完整 template（下方 line 987+）、列齊 **文字 / 換圖 / logo / CTA 顏色與文字 / Footer / Header / 柔邊 / overlay / SEO / UTM 追蹤分享連結 / 裁切 / 歷史版本** 所有免費 post-gen 能力。不列 = 使用者誤以為 LP 不可改、plugin 最大的價值沒傳達。
...
- **UTM 追蹤分享連結**（免費）：要把 LP 發到 IG / FB / EDM / LINE 廣告 / KOL 貼文時、我可以幫你產帶 UTM 參數的分享連結、之後 GA4 / Meta Pixel 才能看出每個渠道帶進來幾個訪客。7 個常見平台（FB / IG / Line / Messenger / Discord / X / Email）我直接套預設、其他渠道（電子報 / Linktree / TikTok 廣告）走自訂。⚠️ UTM 是「外部進來的追蹤標籤」、不是改 LP 內 CTA 按鈕——產出的是新的分享連結讓你貼到別處
...
I) UTM tracking share links → `build_preset_utm_share_url` (FB / IG / Line / Messenger / Discord / X / Email) or `build_utm_share_url` (custom source/medium/campaign) for paste-ready URLs to drop into ads / posts / emails so GA4 / Meta Pixel can attribute inbound traffic per channel; `manage_custom_utms` saves reusable templates per project. Note: UTM does NOT modify the LP's own CTA href — it produces NEW share URLs you paste elsewhere
J) Build homepage → embed this LP into a full website (/salecraft-homepage)
K) Publish → push to social media or run ads (/salecraft-publish)
L) Self-host the LP on your own domain → download stripes + LP config JSON, embed as static HTML block on your own site (see Phase 7 below)
M) Generate another variant (different TA or aspect ratio)
...
- 🧭 **改 header 文字 / footer 文字**（`update_header({text})` / `update_footer({text})`、**免費**）— LP 最上方品牌名跟最下方版權 / 連結文字
- 🔗 **CTA 按鈕連結** — 按鈕連到哪（官網 / LINE / 購買頁 / 預約 / App 下載 / 滑到某區塊、`update_cta_link`、**免費**）
- 🎨 **改 CTA 按鈕樣式** — 顏色 / 圓角 / 陰影（`update_cta_style`、**免費**）
- 🌈 **AI 配色建議** — 不確定 CTA / header / footer 該配什麼色？AI 直接給建議 + WCAG 對比度評分、可一鍵套用最佳建議（`analyze_color_harmony(target="all"/"cta_button"/"header"/"footer", apply=True)`、**免費**）。比喻：「不會配色就交給 AI、它會看你的品牌主色配出對比度合格的搭配、不用自己挑 hex」
- 🔖 **加 UTM 追蹤分享連結** — 想追蹤每個渠道（IG / FB / EDM / LINE 廣告）導流的成效？我幫你產出一條帶 UTM 的分享連結、貼到廣告 / 貼文裡、訪客點進來時 GA4 / Meta Pixel 能歸因（`build_preset_utm_share_url` 預設 7 平台 / `build_utm_share_url` 自訂 / `manage_custom_utms` 存 library 重用、**免費**）。⚠️ 機制提醒：UTM 是「**外部進來的追蹤標籤**」、產出的是**新分享連結**、**不會**改 LP 內部 CTA 按鈕的 href（細節見 `edit-landing` UTM 段）
