...
#### 🔴 新 4-batch 結構（對應 Wizard 6-step、2026-04-20 後定版）

**舊版 LLM 慣性會照這樣問**（❌ 已廢棄）：
- ❌ 批 2 = TA + 長寬比 + 頁數 + 語言 四題一起
- ❌ 批 3 = 色系 + 字體 + CTA + Q&A + 見證 五題一起

**新版正確結構**：

##### 批 1 — Step 2 素材 / 代言人（Gate 1-4）寫入範例

素材組多數在 `brand-onboard` 跑過了（logo / 產品圖 / 認證桶）。這批 update 通常只補 Gate 4 代言人選擇：
...
mcp_tool_call("landing_ai_mcp", "update_session", {
  "user_token": token,
  "session_id": session_id,
  "data_json": json.dumps({"wizard_ta_groups": [<被挑中的 TA objects、flat 結構含 id / ta_group_id / ta_name / ta_description>]})
})
```

##### 批 3 — Step 5 Wizard Phase 2 spec（Gate 6-12）寫入範例
...
mcp_tool_call("landing_ai_mcp", "update_session", {
  ...
  "session_id": session_id,
  "data_json": "{\"wizard_shared_data\": {
    \"aspect_ratio\": \"9:16\",                # Gate 6
    \"cta_url\": \"https://line.me/R/...\",     # Gate 10
    \"cta_text\": \"立即預約\",
    \"include_qa_section\": true,              # Gate 11
    \"include_testimonials\": false            # Gate 12
  }}"
})

# 寫 per-TA（多 TA 時每組可不同）
mcp_tool_call("landing_ai_mcp", "update_session", {
  "user_token": token, "session_id": session_id,
  "data_json": json.dumps({"wizard_ta_groups": [
    {"id": "ta_1", "ta_group_id": "ta_1", "ta_name": "...",
     "language": "en",                        # Gate 7 (per-TA)
     "primary_color": "#2fa067",              # Gate 8 (per-TA)
     "visual_style": "serif_editorial"},      # Gate 9 (per-TA)
    ...
  ]})
})
```

##### 批 4 — Step 6 Wizard Phase 3 頁數（Gate 13 單題）寫入範例
...
mcp_tool_call("landing_ai_mcp", "update_session", {
  "user_token": token,
  "session_id": session_id,
  "data_json": "{\"wizard_shared_data\": {\"requested_stripe_count\": 10}}"
})
# 立刻進 Cost 複誦、等啟動詞
```
...
LLM: (silent tools)
  ...
  ✅ Infer pass（CLAUDE.md #6.5）：aspect/language/color/font/cta/qa/testimonials 從對話 + brand-scrape signal 推
    ↓
互動 1（使用者看 TA 候選、挑組）：
