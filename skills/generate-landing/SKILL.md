...
- ✂️ **裁切某頁**（`crop_stripe` / `reset_crop`、**免費**）— 比喻：「像相機**變焦放大**、只改你看到的範圍、不重新拍」。參數 normalized 0.0-1.0：`{x, y, width, height}`。**注意**：使用者講「從下往上切一段」、要反問「保留上半 / 還是保留下半」、這兩個意思完全相反
- 🖼️ **改背景** — 換色 / 換圖（`update_stripe_background`、**免費**）
- 🧭 **Header 按鈕管理** — LP 最上方那排按鈕：新增 / 刪除 / 調順序 / 改連結（`patch_landing_config` + `get_suggested_buttons`——不確定要放什麼時讓 AI 建議、**免費**）
