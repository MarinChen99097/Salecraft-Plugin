...
| **"幫我做 SEO / GEO / AEO" / "優化 SEO" / "一鍵 SEO" / "改 SEO" / "做 GEO 優化" / "AI 引擎優化" / "答案引擎優化" / "讓 ChatGPT/Perplexity 找得到" / "llms.txt"** | **`run_seo_optimize`** | `force: bool = False`（**AI 一鍵 SEO + GEO + AEO 全包**：meta tags / Open Graph / JSON-LD schema / FAQ schema / **llms.txt（AEO）** / **GEO summary（generative engine）** 一次生齊。預設 500 pts，beta 期免費——呼叫前可用 `seo_preflight` 查實際扣點。**SEO / GEO / AEO 三個 trigger 都走這個 endpoint**、不分開） |
...
| User says | Tool |
|-----------|------|
| "Update the page title" / "改 SEO" / "做 GEO" / "AEO 優化" / "讓 AI 找得到" / any SEO/GEO/AEO intent | `run_seo_optimize` (AI 一鍵 SEO + GEO + AEO 全自動、含 llms.txt + GEO summary、見上方 Page-Level Edits 表) |
...
# 從 detail 讀（注意實際 backend 欄位名）：
#   image_url 或 background_url   — GCS 公開 URL（aliases、用任一個）、永遠是原圖
#   height_px / width_px           — 當前圖實際尺寸
#   original_height_px / original_width_px  — 若 crop 過、這是裁前的原始尺寸
```

**Step 2 — LLM 用 vision 看 image_url、定位使用者要的視覺元素**

LLM 把 `detail["image_url"]`（或 `detail["background_url"]`、同值）餵給 vision：
...
**Step 3 — 換算 CropEditor 座標 + 呼叫 crop_stripe**

CropEditor 容器是 **400×600**（width × height）、backend 用 `editor_scale = min(400/real_w, 600/real_h)` 將圖縮進容器、然後 `actual_px = editor_px / editor_scale`。所以反過來算：要砍真實 X px、要傳 `editor_px = X × editor_scale`。

```python
# 取真實尺寸：優先 original_height_px（若已 crop 過）、否則 height_px
real_h = detail.get("original_height_px") or detail.get("height_px")
real_w = detail.get("original_width_px") or detail.get("width_px")

# 算 editor_scale（跟 backend L3514 一致）
editor_scale = min(400 / real_w, 600 / real_h)

# 要砍真實 80 px、所以 editor px = 80 × editor_scale
real_bottom_to_cut = 80
crop_bottom_px = int(real_bottom_to_cut * editor_scale)

# 範例 1：1200×800 圖、editor_scale=min(0.333,0.75)=0.333、bottom_px=80×0.333=27
# 範例 2：800×1200 圖、editor_scale=min(0.5,0.5)=0.5、bottom_px=80×0.5=40
# 範例 3：400×600 剛好等於容器、editor_scale=1.0、bottom_px=80（直接傳真實 px）

result = mcp_tool_call("landing_ai_mcp", "crop_stripe", {
  "user_token": token, "campaign_id": campaign_id, "stripe_idx": idx,
  "top_px": 0,
...
# result 直接含 original_height + cropped_height + crop_config（不需要再 call get_stripe_detail）
```

**Step 4 — 用 crop_stripe 回傳的 metadata 驗證**

```python
expected_cropped_h = real_h - 80   # 720
actual_cropped_h = result["cropped_height"]   # 直接從 crop_stripe response 讀
# 兩者相差 ±5 px = 算式對、backend 收到正確參數
# 相差大 = 算式錯（很可能是 CropEditor scale 沒算好）→ 走 Step 5
```

**注意**：這只驗證「backend 收到參數、math 對」、**不驗視覺元素是否真的保留好**——那要看 LP 渲染後的視覺（見下方「視覺驗證選項」）。

**Step 5 — 不符預期就 reset + 重算**

不要直接再 call crop（累加破壞）：
```python
reset_crop(campaign_id, stripe_idx)
# 再 call get_stripe_detail 確認 height_px 回到原始
# 重新算 bottom_px、再 crop_stripe 一次、再 verify response.cropped_height
```
...
**Step 2（Fallback）**：照下方公式換算（依該圖實際尺寸）：

```python
detail = get_stripe_detail(...)
real_h = detail.get("original_height_px") or detail.get("height_px")
real_w = detail.get("original_width_px") or detail.get("width_px")
editor_scale = min(400 / real_w, 600 / real_h)

# 使用者要保留上方 keep_ratio（0.0-1.0）：
real_bottom_to_cut = real_h * (1 - keep_ratio)
bottom_px = int(real_bottom_to_cut * editor_scale)
top_px = 0

# 使用者要保留下方 keep_ratio：
top_px = int(real_h * (1 - keep_ratio) * editor_scale)
bottom_px = 0

# 使用者要保留中間 keep_ratio（上下各砍 (1-keep_ratio)/2）：
cut_each_side = real_h * (1 - keep_ratio) / 2
top_px = bottom_px = int(cut_each_side * editor_scale)

# 不裁：top_px = bottom_px = 0
```

Step 3-5 流程同上（用上面算出的 top_px / bottom_px）。
...
## SEO + GEO + AEO Search Optimization — one-click AI（三合一）

`run_seo_optimize` 是**單一按鈕 + AI 全自動**的付費功能（500 pts、beta 期免費）、**同時涵蓋 SEO + GEO + AEO 三個面向**：
- **SEO（Search Engine Optimization）**：Google / Bing / DuckDuckGo 等傳統搜尋引擎 — meta tags、Open Graph、JSON-LD schema、FAQ schema
- **GEO（Generative Engine Optimization）**：ChatGPT / Perplexity / Gemini 等生成式引擎 — `GEO summary` 結構化摘要讓 LLM 抓得到核心訊息
- **AEO（Answer Engine Optimization）**：AI 答案引擎 / Voice search — `llms.txt`（在 root 部署）告訴 LLM 爬蟲怎麼解讀本站

使用者沒有「手動填 title / description / keywords」的路徑——AI 會根據 LP 內容和 Strategist 產出、一次生齊上述全部素材。如果使用者問 GEO / AEO / "讓 ChatGPT 找得到" / "做 llms.txt"、答案都是這個 tool、不分支。
