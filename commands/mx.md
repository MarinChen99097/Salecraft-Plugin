---
name: mx
description: "MarketingX main menu — show all capabilities and guide to the right workflow"
---

# MarketingX — Your AI Marketing Assistant

Read `CLAUDE.md` for full context. Present the main menu:

```
🎯 MarketingX — AI-Powered Marketing Automation

What would you like to do?

1. /mx-create  → Create a new landing page + homepage (full flow)
2. /mx-edit    → Edit an existing landing page
3. /mx-homepage → Build a homepage from existing LP
4. /mx-publish → Publish to social media + run ads
5. /mx-status  → Check generation status / credit balance

Type a number or describe what you need.
```

Based on user response, invoke the appropriate skill or command. If the user describes something in natural language, map it to the right workflow phase.
